...
#fb check mask, want circular mask and cut off the top of the image
...
def update_sensor_local():
    ...
    #sensor_value = (np.sum(LM) > 30) or (np.sum(RM) > 30)
    sensor_value = (np.sum(LM) > 30) or (np.sum(RM) > 30) #fb invert values to trigger threshold
    # print("UPDATED")
...
#         # leds.on()
#         rotate_left(0.5, 6, 0.4, 0.3)
#     else:
#         # leds.on()
#         rotate_right(0.5, 6, 0.4, 0.3)
...
for i in range(60):
    photodiode.update()
    if (photodiode.brightness > 400):
        break
    
    rotate_right(0.5, 8, 6, 0.4, 0.3)
