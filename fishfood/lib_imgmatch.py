...
class ImgMatch():
    def __init__(self, no_images, max_blobs, dist_thresh, light_sens, cont_pix):
    ...
    def _blob_detection(self, imgs, side):
        ...
        for ii in range(self.no_images):
            if side == 'right':
                self.blob_r.detect(imgs[ii], self.cont_pix)
                self.blobs.append(self.blob_r.blobs.tolist()) # make python list
            else:
                self.blob_l.detect(imgs[ii], self.cont_pix)
    ...
    def _oneA_to_allB_outliers(self, imgA, imgB):
    ...
    def _outlier_detection(self):
        ...
        for ii in range(self.no_images-1):
            ...
            #imgA, imgB = self._remove_img_edge(imgA, imgB)
            ...
            #new_outliers = self._oneA_to_allB_outliers(imgA, imgB)
            new_outliers = self._allA_to_allB_outliers(imgA, imgB)
